...
<div class="page-wrapper">
	<div class="row page-titles">
        ...
        <?php if(isset($_GET['eid']) && $_GET['eid'] != ''){?>
          <li class="breadcrumb-item"><a href= "{{route('restaurants.reviews',$_GET['eid'])}}" >{{trans('lang.order_review')}}</a></li>
        <?php }else{ ?>
          <li class="breadcrumb-item"><a href= "{!! route('orderReview') !!}" >{{trans('lang.order_review')}}</a></li>
        <?php } ?>
				
				<li class="breadcrumb-item active">{{trans('lang.food_review_edit')}}</li>
			</ol>
		</div>
...
<div>
      ...
      <div class="card-body">
        ...
        <div class="row restaurant_payout_create">
          ...
          <!-- <div class="col-md-6"> -->
            <fieldset>
              <legend>{{trans('lang.food_review_edit')}}</legend>
            
              <div class="form-group row width-100">
                <label class="col-3 control-label">{{ trans('lang.food_review_user_id')}}</label>
                <div class="col-7">
              ...
              <div class="form-group row width-100">
                <label class="col-4 control-label">{{trans('lang.restaurant')}}</label>
                <div class="col-7"> 
              ...
              <div class="form-group row width-100">
                  <label class="col-3 control-label">{{ trans('lang.food_review_rate')}}</label>
                  <div class="col-7">
      ...
      <div class="form-group col-12 text-center btm-btn">
        ...
        <?php if(isset($_GET['eid']) && $_GET['eid'] != ''){?>            
          <a href="{{route('restaurants.reviews',$_GET['eid'])}}" class="btn btn-default"><i class="fa fa-undo"></i>{{trans('lang.cancel')}}</a>
        <?php }else{ ?>
        ...
        return false;    
         ...
         database.collection('foods_review').doc(id).update({'comment':user_review,'rating':rate,'CustomerId':user_id,'VendorId':vendor_id}).then(function(result) {
                ...
                <?php if( isset($_GET['eid']) && $_GET['eid'] !=''){ ?>
                     ...
                     window.location.href = "{{ route('restaurants.reviews',$_GET['eid']) }}";                    
                <?php  }else{ ?>
