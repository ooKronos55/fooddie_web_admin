...
<div class="page-wrapper">
  ...
  <div>
    ...
    <div class="form-group col-12 text-center btm-btn">
        ...
        <?php if(isset($_GET['eid']) && $_GET['eid'] != ''){?>            
            <a href="{{route('restaurants.coupons',$_GET['eid'])}}" class="btn btn-default"><i class="fa fa-undo"></i>{{trans('lang.cancel')}}</a>
        <?php }else{ ?>
  ...
  console.log('resturant_id '+resturant_id);
      ...
      /*}else if(resturant_id == ''){
       ...
       //   console.log('done'+resturant_id+' '+expiresAt);
        ...
        database.collection('coupons').doc(id).update({'code':code,'description':description,'discount':discount,'expiresAt':expiresAt,'isEnabled':isEnabled,'id':id,'discountType':discountType,'image':photo_coupon,'resturant_id':resturant_id}).then(function(result) {
